"""
Run the v3 centering pipeline with the fine-tuned Cellpose model on all 30
Categorised_Data files. Same logic as process_all_centering.py but with the
fine-tuned model instead of off-the-shelf cyto3.

Output goes to a separate directory so we can compare against the original
v3 outputs side-by-side.

The fine-tuned model path is set via FINETUNED_MODEL_PATH (override before
running if Cellpose saved the model with a different name).
"""
import json
import os
import time
...
OUTPUT_DIR = Path('/dcs/pg25/u1898019/Desktop/Categorised_Data_finetuned')
TRAIN_DIR = Path('/dcs/pg25/u1898019/Desktop/Categorised_Data_centered/training_data')

# Finetune-time Cellpose saves the model under a `models/` subfolder of the
# training directory. Pick the most recent file there at runtime.
...
def process_file(input_path: Path, out_centered: Path, out_mask: Path) -> dict:
    ...
    for t in range(T):
        ...
        # CellposeModel.eval returns (masks, flows, styles) -- 3 elements (no diams)
